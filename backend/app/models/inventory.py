...
class Inventory(Base, IdIntPk, TimeStampMixin):
    ...
    # Уникальный аппаратный идентификатор (MAC). Необязателен: не у каждого
    # устройства он известен, но если задан — не должен повторяться.
    mac_address: Mapped[Optional[str]] = mapped_column(String, unique=True, nullable=True)
